        ...
        foreach ($registros as $fila) {
            ...
            $consulta .= '<table class="table modern-table table-hover" id="tablaPrincipal">
                        ...
                        <tbody>
                            ...
                            <tr>
                        <td>' . $referencia . '</td><td>' . $descripcion . '</td><td>' . $existencias . '</td><td>' . $existencias_bog . '</td><td>' . $fila['Marca'] . '</td><td>$' . number_format($precio_con_descuento, 0, '.', ',') . '</td>
                        </tr>
    ...
    if ($resultuse && !empty($resultuse) > 0 && $entra != 1) {
        ...
        if ($stmtP && $stmtP->rowCount() > 0) {
           ...
           <caption style="background-color:#b2dcff;font-weight:bold"">Consulta de Inventario</caption>
                    ...
                    <tbody>
            <tr>
            <td>' . $referenciap . '</td><td>' . $descripcion . '</td><td>' . $existencias . '</td><td>' . $existencias_bog . '</td><td>' . $fila['Marca'] . '</td><td>$' . number_format($precio_con_descuento, 0, '.', ',') . '</td>
            </tr>
       ...
       <?php
            ...
            <?php
                ...
                foreach ($arregloAlternos['alternos'] as $valor) {
                    ...
                    $sql = $con->prepare("SELECT * FROM inventario LEFT JOIN marcas ON inventario.Marcas = marcas.Proveedor WHERE referencia = :referencia and Tipo='agro' ");
