...
function Existencias($usuario, $resultado_alterno, $resultuse, $resultcostex, $con, $referencia)
{
    ...
    if ($resultado_alterno && !empty($resultado_alterno)) {
        ...
        $consulta .= '<table class="table modern-table table-hover" id="tablaPrincipal">'.
                    (($fila['Bodega'] == '2') ? '<caption style="background-color:#86bdb5">Existencias en Bogota</caption>' : '<caption style="background-color:#b2dcff">Existencias en Barranquilla</caption>') .
                    ...
                    <tbody>
                        ...
                        <tr>
                    <td>' . $referencia . '</td><td>' . $descripcion . '</td><td>' . $existencias . '</td><td>$' . number_format($precio_con_descuento, 2, '.', ',') . '</td>
                    </tr>
    ...
    global $descuento;
    global $precio_con_descuento;
    $sqlPedidos = "SELECT * FROM pedidos a inner join inventario b on a.referencia=b.Referencia WHERE a.referencia = :referencia";
    ...
    if ($stmtPedidos && $stmtPedidos->rowCount() > 0) {
        ...
        while ($fila = $stmtPedidos->fetch(PDO::FETCH_ASSOC)) {
           // $precioPedido = 0;
            ...
            $Pedidos .= '<tr><td>' . $fila['IdPedido'] . '</td><td>' . $fila['referencia'] . '</td><td>' . $fila['Cantidad'] . '</td><td>' . $fila['FechaPedido'] . '</td> <td>' .'$'. number_format($precioPedido,  0, '.', ',').'</td> </tr>';
        }
    ...
    <div class="container">
       ...
       <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        if (isset($_POST['buscar']) || isset($_GET['referencia'])) :
            ...
            $sql = $con->prepare("SELECT * FROM inventario WHERE referencia = :referencia and Tipo='agro' order by bodega asc");
        ...
        <table class="table modern-table table-hover" id="tablaPrincipal" style="border:none">
            ...
            <tbody>
                ...
                foreach ($arregloAlternos['alternos'] as $valor) { 
                    ...
                    $sql->execute(['referencia' => trim($comentario[0] ?? '')]);
                    ...
                    if ($resultado && count($resultado) > 0) {
                        ...
                        $existenciasAlterno = $fila['Existencias'];
                    ...
                    if ($existenciasAlterno>0)
                    {
                        ...
                        $mensaje = '<a href="consulta_inventario.php?referencia='.trim($comentario[0] ?? '').'"> <button type="button" class="btn btn-primary" style="font-size: 12px; padding: 5px 5px;">Ver pedidos</button> </a>';
                    ...
                    <tr>
                        ...
                        <td>Alterno</td>
                        <td><?php echo $existenciasAlterno ?></td>
        ...
        <?php
                ...
                if (($tipo_busqueda == "descripcion" ))
                {
                    ...
                    */
                    ...
                    $sql = $con->prepare("SELECT Referencia, descripcion, sum(Existencias) as Existencias, MAX(Precio) AS Precio, MAX(Linea) AS Linea  FROM inventario WHERE $where AND Tipo <> 'use' group by Referencia, descripcion LIMIT $limite");
                    $sql->execute($params);
                    ...
                    if ($resultado && count($resultado) > 0) {
                        ...
                        foreach ($resultado as $fila) {
                            ...
                            if ($fila['Existencias']>0)
                            {
                                ...
                                echo '<a href="consulta_inventario.php?referencia='.$fila['Referencia'] . '&tipo_busqueda='.$tipo_busqueda.'"><div class="custom-row row hover-row"><div class="col">' . $fila['Referencia'] . '</div><div class="col">' . $fila['descripcion'] . '</div> <div class="col">' . $fila['Existencias'] . '</div> <div class="col">' .'$'.number_format($precioADesccripcion , 0, '.', ',') . '</div></div></a>';

                            }
                            else{

                                 echo '<a href="consulta_inventario.php?referencia='.$fila['Referencia'] . '&tipo_busqueda='.$tipo_busqueda.'"><div class="custom-row row hover-row"><div class="col">' . $fila['Referencia'] . '</div><div class="col">' . $fila['descripcion'] . '</div> <div class="col">' . $fila['Existencias'] . '</div> <div class="col"> <button type="button" class="btn btn-primary" style="font-size: 12px; padding: 5px 5px;">Ver pedidos</button> </div></div></a>';
...
if (pedidos!== null) {
        ...
        var precioPedido = pedidos.rows[1].cells[4].textContent;
        ...
        if (disponible.includes('60 DIAS')) {
            ...
            tablaPrincipal.rows[1].cells[3].innerHTML=precioPedido; 
