       ...
       <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        ?>
                <br>
                    ...
                    <?php
                    //empiezo aqui
                    // buscar si existe una imagen asociada a la referencia
                    $extensiones = ['png', 'jpg', 'jpeg', 'gif'];
                    $ruta_imagen = '';
                    foreach ($extensiones as $ext) {//recorremos
                        $ruta_fisica = __DIR__ . "/imagenes_items/$referencia.$ext";
                        $url_imagen = "imagenes_items/$referencia.$ext";
                        ...
                        <table class="table modern-table table-hover" id="tablaPrincipal">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    echo Existencias($_SESSION['idusuario'], $resultado_alterno[0], $resultuse[0], $resultcostex[0], $con);

                                    // mostrar iono solo si hay imagen
                                    if (!empty($ruta_imagen)) {
                                        echo "<td>
                                                <a href='#' data-bs-toggle='modal' data-bs-target='#modalImagen' style='text-decoration:none; display:inline-block;'>
                                                    <img src='imagenes_items/iconos/icono_imagen.png' alt='Ver Imagen' style='width:28px; height:28px; cursor:pointer;'>
                                                </a>
                                            </td>";
                                    }
                                    ?>
...
<!-- Modal Bootstrap popo-up de imagenes de piezas-->
<div class='modal fade' id='modalImagen' tabindex='-1' aria-labelledby='modalImagenLabel' aria-hidden='true'>
    ...
    <div class='modal-content'>
      ...
      <div class='modal-header'>
        ...
        <button type='button' class='btn' data-bs-dismiss='modal' 
                style="font-size:28px; font-weight:bold; color:black; background:none; border:none; line-height:1;">
          ×
        </button>
      </div>

      <!-- Cuerpo del modal con Panzoom -->
      <div class='modal-body text-center' style="overflow:hidden;">
        <div id="panzoom-container" style="display:inline-block; cursor: grab;">
          <img id="imagenZoom" src='<?php echo $ruta_imagen; ?>' 
               alt='Imagen Producto' 
 ...
 <?php else : ?>
        ...
        <?php
                ...
                if ($tipo_busqueda == "descripcion" && ($_SESSION['idusuario'] == "ADMIN"))
                {
...
document.addEventListener("DOMContentLoaded", function() {
  const contenedor = document.getElementById('panzoom-container');
  if (contenedor) {  // Evita errores si no hay imagen
    const panzoom = Panzoom(contenedor, {
      maxScale: 5,
      minScale: 1,
      contain: 'outside'
    });
    ...
    // Habilitar doble clic para zoom
    contenedor.addEventListener('dblclick', function() {
      panzoom.zoomIn({ animate: true });
    });

    // Cambiar cursor al arrastrar
    const imagen = document.getElementById('imagenZoom');
    imagen.addEventListener('mousedown', () => imagen.style.cursor = 'grabbing');
    document.addEventListener('mouseup', () => imagen.style.cursor = 'grab');
  }
